...
2. [create_testset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_testset.py:0:0-0:0) でテストセット（質問＋理想回答）を作る  
3. [create_dataset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_dataset.py:0:0-0:0) で RAG を実行した結果付きデータセットを作る  
4. [evaluate_rag.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/evaluate_rag.py:0:0-0:0) で Ragas による評価を行う  
...
RAG 本体（前回記事の `rag_opensearch/`）に加えて、評価用のコードは [rag_evaluate/](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate:0:0-0:0) にまとめています。
...
1. **Step 1: PDF → Markdown チャンク生成（[pdf2md_per_pages.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/pdf2md_per_pages.py:0:0-0:0)）**  
   - 入力: `docs/*.pdf`  
   - 出力: `rag_evaluate/pdf2md_per_pages/*.md`
2. **Step 2: テストセット生成（[create_testset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_testset.py:0:0-0:0)）**  
   - 入力: Step 1 の Markdown チャンク（`PDF2MD_OUTPUT_DIR`）  
   - 出力: `rag_evaluate/testsets/testset_YYYYMMDDHHMMSS.csv`
3. **Step 3: 評価用データセット作成（[create_dataset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_dataset.py:0:0-0:0)）**  
...
4. **Step 4: RAG を Ragas で評価（[evaluate_rag.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/evaluate_rag.py:0:0-0:0)）**  
   - 入力: Step 3 のデータセット CSV（`EVAL_DATASET_CSV_PATH`）  
   - 出力: メトリクス平均＋詳細 CSV（`rag_evaluate/eval_results/*.csv`）

途中の CSV パスはすべて [rag_evaluate/config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/config.py:0:0-0:0) に集約しているため、  
...
### 4. [rag_evaluate/config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/config.py:0:0-0:0) を確認
...
## Step 1: PDF → Markdown チャンク生成 (pdf2md_per_pages)

まず、評価対象となる PDF（例: 行政レポートや観光動向調査など）を `docs/` 配下に置きます。

- 入力: `docs/*.pdf`（`PDF2MD_INPUT_PATTERN`）  
- 出力: `rag_evaluate/pdf2md_per_pages/*.md`（`PDF2MD_OUTPUT_DIR`）  
- 変換単位: `PDF2MD_PAGES_PER_CHUNK` ページごと
...
[create_testset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_testset.py:0:0-0:0) の中では、ざっくり次のような処理をしています。

- [pdf2md_per_pages/](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/pdf2md_per_pages:0:0-0:0) 以下の Markdown を読み込み、Knowledge Graph を構築
...
この CSV のパスを、[rag_evaluate/config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/config.py:0:0-0:0) の `EVAL_INPUT_CSV` に設定しておきます。
...
[create_dataset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_dataset.py:0:0-0:0) では、次のようなことをしています。
...
最終的に、Ragas の `EvaluationDataset` にそのまま渡せる形の CSV を [datasets/](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/datasets:0:0-0:0) に保存します。

この CSV のパスを、[rag_evaluate/config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/config.py:0:0-0:0) の `EVAL_DATASET_CSV_PATH` に設定します。
...
[evaluate_rag.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/evaluate_rag.py:0:0-0:0) では、
...
```text
📄 入力CSV: /home/ouchi/for_blog/rag_evaluate/testsets/testset_20251201101757.csv
✓ テストセット読み込み完了: 5件
...
=== RAG実行完了 ===
✅ データセットを保存しました: /home/ouchi/for_blog/rag_evaluate/datasets/dataset_20251201_111112.csv
   レコード数: 5
```
...
- **4 ステップの実行順序が前提になっている**  
  - [pdf2md_per_pages](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/pdf2md_per_pages:0:0-0:0) → `create_testset` → `create_dataset` → `evaluate_rag`  
...
- 設定値（インデックス名・モデル名・ファイルパス）は **すべて [config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/pdf2md/config.py:0:0-0:0) に寄せている**  
  - 実装側は「config から読むだけ」にし、実験時は [config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/pdf2md/config.py:0:0-0:0) を書き換える運用
- 評価用 LLM（`gpt-5-mini`）と RAG 用 LLM（`gemini-2.5-pro`）を役割分担させている  
...
### 1. PDF → Markdown 変換（[pdf2md_per_pages.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/pdf2md_per_pages.py:0:0-0:0)）

- [split_pdf_into_chunks()](cci:1://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/pdf2md_per_pages.py:39:0-85:26) で PDF を `PDF2MD_PAGES_PER_CHUNK` ページごとの一時 PDF に分割
...
### 2. テストセット生成（[create_testset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_testset.py:0:0-0:0)）
...
### 3. データセット作成（[create_dataset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_dataset.py:0:0-0:0)）

- `get_opensearch_rag()` で前回記事の RAG 実装を再利用
- [run_rag_on_testset()](cci:1://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_dataset.py:55:0-126:18) でテストセットの各行に対して RAG を実行し、  
  - `response`（RAG の回答）  
...
### 4. RAG 評価（[evaluate_rag.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/evaluate_rag.py:0:0-0:0)）

- `METRIC_REGISTRY` で、文字列のメトリクス名 → 実際のクラス  
  のマッピングを管理
- `EVAL_METRICS` に指定した名前だけを [build_metrics()](cci:1://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/evaluate_rag.py:78:0-85:18) でインスタンス化
- 新しいメトリクスを試したいときは、  
...
- パイプラインは  
  1. PDF → Markdown チャンク ([pdf2md_per_pages.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/pdf2md_per_pages.py:0:0-0:0))  
  2. テストセット生成 ([create_testset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_testset.py:0:0-0:0))  
  3. 評価用データセット作成 ([create_dataset.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/create_dataset.py:0:0-0:0))  
  4. RAG 評価 ([evaluate_rag.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/rag_evaluate/evaluate_rag.py:0:0-0:0))  
...
あとは、使いたいメトリクスや評価用 LLM を [config.py](cci:7://file://wsl.localhost/Ubuntu/home/ouchi/for_blog/pdf2md/config.py:0:0-0:0) や `EVAL_METRICS` で入れ替えつつ、  
